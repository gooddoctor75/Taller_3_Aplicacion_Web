...
router.get('/', (req, res) => {
    const sql = `SELECT p.id, p.name, p.description, u.username as owner
...
router.post('/', verifyToken, (req, res) => {
    ...
    db.run(sql, [name, description, req.user.id], function (err) {
        if (err) return res.status(500).json({ error: err.message });
        res.status(201).json({ id: this.lastID, name, description, owner_id: req.user.id });
    });
});
...
router.put('/:id', verifyToken, (req, res) => {
    ...
    db.get(checkOwnerSql, [productId], (err, product) => {
        ...
        if (product.owner_id !== req.user.id) return res.status(403).json({ error: 'No autorizado.' });
