...
router.post('/register', (req, res) => {
    ...
    if (!username || !password) return res.status(400).json({ error: 'Se requieren nombre de usuario y contraseña.' });
...
router.post('/login', (req, res) => {
    ...
    db.get(sql, [username], (err, user) => {
        ...
        bcrypt.compare(password, user.password_hash, (err, result) => {
            if (err || !result) return res.status(401).json({ error: 'Credenciales inválidas.' });
